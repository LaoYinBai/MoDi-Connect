...
internal static class BtPassiveHandshake
{
    ...
    private const string BtToken = "MODI";  // 必须 ≤ 8 字符（payload 限制）
    ...
    /// 返回 route（0-3），失败返回 -1。
    /// </summary>
    public static async Task<HandshakeResult?> WaitForHelloAsync(BluetoothTransport transport, CancellationToken ct)
    {
        ...
        Action<ReadOnlyMemory<byte>> handler = data =>
        {
            ...
            if (!HelloSessionPayload.TryDecode(decoded.Value.Payload, tokenRequired: true, out var identity) ||
                identity.Token != BtToken)
            {
                Log.W(Tag, "Invalid session payload or token mismatch");
                var nack = new Packet { Type = PacketType.HelloNack, LinkType = LinkType.Bluetooth, Sequence = 0, Payload = Array.Empty<byte>() };
            ...
            var ack = new Packet { Type = PacketType.HelloAck, LinkType = LinkType.Bluetooth, Sequence = 0, Payload = ackPayload };
