...
/**
 * UsbHandshakeClient — USB 链路主动握手（Android 端）
 *
 * 职责：构造 HELLO(route+token) → 发送 → 等待 HELLO_ACK。
 * 与 BtHandshakeClient 完全对称（仅 LinkType 不同）。
 */
object UsbHandshakeClient {

    private const val TAG = "UsbHandshake"
    private const val USB_TOKEN = "MODI"  // 必须 ≤ 8 字符（payload 限制）
    ...
    suspend fun sendHelloAndWaitForAck(
        ...
        val payload = HelloSessionPayload.encode(route, USB_TOKEN, sessionId)

        val packet = Packet(PacketType.HELLO, LinkType.USB, 0u, payload)
