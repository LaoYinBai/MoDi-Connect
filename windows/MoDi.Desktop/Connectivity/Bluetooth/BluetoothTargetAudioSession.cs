...
/// <summary>Dev-gated session/channel projection for an already authenticated RFCOMM connection.</summary>
internal sealed class BluetoothTargetAudioSession
{
    ...
    internal BluetoothTargetAudioSession(Guid sessionUuid, ITransport legacy)
    {
        ...
        var peerId = PeerId.Parse($"bluetooth-session:{sessionUuid:N}");
        Peer = new Peer(peerId, null, new HashSet<ConnectivityCapability> { ConnectivityCapability.Audio });
        Session = new Session(_id, peerId, TransportKind.Bluetooth, SessionState.Connecting);
        _transport = new ConnectedTransportSession(TransportKind.Bluetooth, legacy);
        _channel = new LegacyAudioChannelAdapter(_id, _transport);
        AudioTransport = new ChannelTransportAdapter(_channel, TransportType.Bluetooth);
    }
    ...
    {
        if (_bound) return;
        Sessions.ObserveStarted(_id, TransportKind.Bluetooth, SessionState.Connecting);
