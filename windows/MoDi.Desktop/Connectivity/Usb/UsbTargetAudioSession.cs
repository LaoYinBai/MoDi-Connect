...
/// <summary>Dev-gated session/channel projection for an already authenticated private ADB tunnel.</summary>
internal sealed class UsbTargetAudioSession
{
    ...
    internal UsbTargetAudioSession(Guid sessionUuid, ITransport legacy)
    {
        ...
        var peerId = PeerId.Parse($"usb-session:{sessionUuid:N}");
        Peer = new Peer(peerId, null, new HashSet<ConnectivityCapability> { ConnectivityCapability.Audio });
        Session = new Session(_id, peerId, TransportKind.Usb, SessionState.Connecting);
        _transport = new ConnectedTransportSession(TransportKind.Usb, legacy);
        _channel = new LegacyAudioChannelAdapter(_id, _transport);
        AudioTransport = new ChannelTransportAdapter(_channel, TransportType.Usb);
    }
    ...
    {
        if (_bound) return;
        Sessions.ObserveStarted(_id, TransportKind.Usb, SessionState.Connecting);
