...
class AudioPipeline(
    ...
    // ── 启动推流 ──
    suspend fun startStreaming(m: Int = MODE_MIC, proj: MediaProjection? = null, ctx: Context? = null, host: String? = null, port: Int = TransportIdentity.AUDIO_PORT, localBindAddress: String? = null): Boolean {
     ...
     */
    suspend fun startStreamingWithTransport(t: ITransport, m: Int, proj: MediaProjection? = null, ctx: Context? = null): Boolean {
        if (isStreaming()) return true
    ...
    // ── 推流中切换采集模式 ──
    suspend fun switchMode(m: Int, proj: MediaProjection? = null, ctx: Context? = null): Boolean {
        ...
        withContext(ioDispatcher) { captureLoop.stop(releaseSystemAudio) }
        encodeSender.reset()
        mode = m
        return withContext(ioDispatcher) { captureLoop.start(m, proj, ctx) }
    }

    // ── 停止推流 ──
    suspend fun stopStreaming() {
        withContext(ioDispatcher) { captureLoop.release() }
        encodeSender.release()
        Log.i(TAG, "stopped")
    }
